...
public class Main {
    public static void main(String[] args) {
        ...
        try(BufferedReader r = new BufferedReader((new FileReader(pathID)))) { /
            String line;
        ...
        for (int i = 0; i < n; i++) { // заполняем двуммерный массив
        ...
        ArrayList<Integer> watched = new ArrayList<Integer>(); // список фильмов
        for (int i = 0; i < inputID.length; i++)
            watched.add(Integer.parseInt(inputID[i]));

        ArrayList<Integer> sameInterest = new ArrayList<>();
